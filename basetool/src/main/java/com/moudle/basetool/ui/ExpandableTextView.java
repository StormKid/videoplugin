...
public class ExpandableTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    public ExpandableTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        DEFAULT_TAIL_TEXT_COLOR = ContextCompat.getColor(context, R.color.text_666);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ExpandableTextView);
        mFoldText = a.getString(R.styleable.ExpandableTextView_foldText);
        if (null == mFoldText) {
        ...
        mFoldLine = a.getInt(R.styleable.ExpandableTextView_foldLine, DEFAULT_FOLD_LINE);
        if (mFoldLine < 1) {
            throw new RuntimeException("foldLine must not less than 1");
        }
        mTailColor = a.getColor(R.styleable.ExpandableTextView_tailTextColor, DEFAULT_TAIL_TEXT_COLOR);
        mCanFoldAgain = a.getBoolean(R.styleable.ExpandableTextView_canFoldAgain, DEFAULT_CAN_FOLD_AGAIN);
        ...
        if (!mIsFold) {
            ...
            if (line < layout.getLineCount()) {
                ...
                if (index > 0) {
                    ...
                    LogUtil.Companion.d(TAG, "strWhichHasExactlyFoldLine-->" + strWhichHasExactlyFoldLine);
    ...
    protected void onDraw(Canvas canvas) {
        LogUtil.Companion.d(TAG, "onDraw() " + mCountOnDraw++ + ", getMeasuredHeight() " + getMeasuredHeight());
    ...
    private SpannableString createFoldSpan(String text) {
        ...
        LogUtil.Companion.d(TAG, (System.currentTimeMillis() - startTime) + "ms");
    ...
    private String tailorTextBackUp(String text) {
        LogUtil.Companion.d(TAG, "使用备用方法: tailorTextBackUp() " + mCountBackUp++);
    ...
    private String tailorText(String text) {
        ...
        while (find != 0 && end > start) {
            LogUtil.Companion.d(TAG, "使用二分法: tailorText() " + mCountBinary++);
            if (find > 0) {
                end = mid - 1;
            } else if (find < 0) {
                start = mid + 1;
        ...
        LogUtil.Companion.d(TAG, "mid is: " + mid);
